theory SmartCityValueOntologies_Clean (*\<copyright> A. Vestrucci 2025*)
  imports Main
begin
...
axiomatization where (*promotes_in_set: "promotes P \<in> promotes_set P" and*)
  set_nonempty:    "promotes_set P \<noteq> {}"
...
(********************************************************************************************************************)
text \<open>Ontology 1: A fixed value order.
...
lemma
  ...
  and "\<forall> Q. \<not> A1 \<in> Iset2 Q" and "B1 \<notin> \<Gamma>"
      ...
      and "\<forall>v. v \<in> Iset2 Q \<and> v \<notin> \<Gamma> \<longrightarrow> A1 \<succ>v v"
...
text  \<open>In ontology 2 things are different: The preference between projects varies because 
it is parametrized by conditions (e.g. a smart city wants to promote a specific value and 
